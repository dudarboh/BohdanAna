...
df = ROOT.RDataFrame("treename", "/nfs/dust/ilc/user/dudarboh/tof/BohdanAna.root").Filter("abs(pdg) == 211")
df = df.Filter("tofClosest0 > 6.")
# ROOT.Experimental.AddProgressBar(df)
n_mass_bins, mass_min, mass_max = 500, -0.5, 0.5

tof_columns = [f"tofClosest{i*20}" for i in range(6)]
histos = []
...
for i, h in enumerate(histos):
    ...
    h.SetLineWidth(4)
    legend.AddEntry(h.GetPtr(), f"{i*20} ps","l")
    if i == 0:
...
line = ROOT.TLine(0.13957039, 0., 0.13957039, 110000.)
line.Draw()
line.SetLineWidth(3)
line.SetLineStyle(9)
